...
class Teeple_FileUtil
{
    ...
    function ls($dirName)
    {
        ...
        while (($file = readdir($dh)) !== false) {
            ...
            $fullPath = self::_addTail($dirName, DIRECTORY_SEPARATOR).$file;
            if (is_dir($fullPath)) {
                $dirs[] = $fullPath;
            } else {
                $files[] = $fullPath;
    ...
    function findRecursive($dirName, $regex = "")
    {
        ...
        $found = self::find($dirName, $regex);

        foreach ($dirs as $dir) {
            $found = array_merge($found, self::findRecursive($dir, $regex));
        }
