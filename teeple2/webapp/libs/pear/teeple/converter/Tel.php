...
/**
 * 電話番号を1つにまとめます。(ハイフンで結合)
 * 電話番号はそれぞれ、$foo[0],$foo[1],$foo[2]に格納されている前提です。
 * target属性で指定されたフィールド名に値を格納します。
 *
 * @package teeple.converter
 */
class Teeple_Converter_Tel extends Teeple_Converter
{
    ...
    protected function execute(&$obj, $fieldName) {
        ...
        if ($value[0] != "" && $value[1] != "" && $value[2] != "") {
            $newvalue = sprintf("%s-%s-%s", $value[0], $value[1], $value[2]);
